...
![ZygoteInit](https://raw.githubusercontent.com/keelim/AOSP/master/docs/assets/zy4.png)

### (1) 새로운 Android 애플리케이션의 실행 요청을 받기 위한 소켓 바인딩

- registerZygoteSocket()
- UDS(Unix Domain Socket)을 사용하여 애플리케이션 실행을 위한 요청을 수신
- init 프로세스가 init.rc 파일에서 app_process를 실행할 때 /dev/zygote로 등록한 socket을 사용함

### (2) Android Application Framework에서 사용할 클래스들과 리소스의 로딩

- preloadClasses()
- preloadResources()  
- 애플리케이션 프레임워크에 포함된 클래스와 플랫폼 자원(이미지, XML 정보, 문자열 등)을 미리 메모리 로딩
- 로딩된 클래스와 자원은 새로 생성되는 프로세스에서 다시 로딩하는 것이 아니라 그대로 사용되므로 애플리케이션이 빠르게 생성
...
![ZygoteOverview](https://raw.githubusercontent.com/keelim/AOSP/master/docs/assets/zy5.png)
