...
- 활용 예  
  ...
  - 이 경우 호출 API를 이용해 기존 프로그램과 자바 프로그램을 하나의 프로그램으로 만들 수 있음
...
main() 메서드는 정적 메서드로 문자열 객체 배열을 인자로 받아 그 중 첫 번째 문자열을 가리키는 `args[0]`을 화면에 출력
...
jni.h 파일을 포함
이 헤더 파일에는 C 코드에서 JNI를 사용하는 데 필요한 각종 변수 타입, JNI 함수가 정의
